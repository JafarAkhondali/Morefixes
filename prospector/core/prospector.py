...
#TIME_LIMIT_BEFORE = 500 * SECS_PER_DAY
TIME_LIMIT_BEFORE = 60 * SECS_PER_DAY
TIME_LIMIT_AFTER = 60 * SECS_PER_DAY
#TIME_LIMIT_AFTER = 500 * SECS_PER_DAY
...
def prospector(  # noqa: C901
    ...
    if len(candidates) > limit_candidates:
        # Pick top-nearest commit candidates related to advisory publish date
        sorted_candidates = sorted(list(candidates.values()), key=lambda raw_commit: abs(raw_commit.timestamp-advisory_record.reserved_timestamp))
        sorted_candidates = sorted_candidates[:limit_candidates]
        candidates = {}
        for c in sorted_candidates:
            candidates[c.id] = c

        logger.warning(f"Number of candidates exceeds {limit_candidates}.")
        # rules = [
        #     'VULN_ID_IN_MESSAGE',
        #     'XREF_BUG',
        #     'XREF_GH',
        ...
        ConsoleWriter.print(
            f"Candidates limit exceeded, limited to nearest commits related to advisory.",
        )
        # return None, len(candidates)


    since_valid = advisory_record.reserved_timestamp - (time_limit_before*9)
    until_valid = advisory_record.reserved_timestamp + (time_limit_after*9)


    for ref in advisory_record.references:
        if ref.startswith('commit::'):
            try:
                mentioned_commit = list(repository.create_commits(ref.replace('commit::', '')).values())[0]
            except Exception as e:
                continue
            print(since_valid, mentioned_commit.timestamp, until_valid)
            if since_valid <= mentioned_commit.timestamp <= until_valid:
                print(f'Adding mentioned commit {mentioned_commit.id}')
                candidates[mentioned_commit.id] = mentioned_commit

#    for ref in advisory_record.references:
#        if ref.startswith('commit::'):
#            mentioned_commit = list(repository.create_commits(ref.replace('commit::', '')).values())[0]
#            print(since_valid, mentioned_commit.timestamp, until_valid, mentioned_commit.id)
#            if since_valid <= mentioned_commit.timestamp <= until_valid:
#                print(f'Adding mentioned commit {mentioned_commit.id}')
#                candidates[mentioned_commit.id] = mentioned_commit
